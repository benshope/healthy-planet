<?php
//comment form submit
function prestige_comment_form()
{
    ...
    if(isEmpty($values['comment-user-name']) || strcmp($values['comment-user-name'], __('Your Name', 'prestige'))==0)
    {
        $response['error']=1;
        $response['info'][]=array('fieldId'=>'comment-user-name','message'=> __('Please enter your name', 'prestige'));
    }

    if(!validateEmail($values['comment-user-email']) || strcmp($values['comment-user-email'], __('Your Email Address', 'prestige'))==0)
    {
        $response['error']=1;	
        $response['info'][]=array('fieldId'=>'comment-user-email','message'=> __('Please enter valid e-mail address', 'prestige'));
    }

    if(isEmpty($values['comment-message']) || strcmp($values['comment-message'], __('Message', 'prestige'))==0)
    {
        $response['error']=1;
        $response['info'][]=array('fieldId'=>'comment-message','message'=> __('Please enter your message', 'prestige'));
    }	
	...
	{
		$response['error']=0;
		$response['info'][]=array('fieldId'=>'comment-send','message'=> __('Your comment has been added', 'prestige'));
		//get post comments
		//post
	...
	{
		$response['error']=1;	
        $response['info'][]=array('fieldId'=>'comment-send','message'=> __('Error while adding comment', 'prestige'));
        	
	}
	createResponse($response);
}
add_action("wp_ajax_prestige_comment_form", "prestige_comment_form");
add_action("wp_ajax_nopriv_prestige_comment_form", "prestige_comment_form");
?>