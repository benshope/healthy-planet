...
function portfolio_posts_per_page( $query ) {
    ...
    if ( !empty($query->query_vars['post_type']) && $query->query_vars['post_type'] == 'portfolio' ) {
        if (is_archive())
            $query->query_vars['posts_per_page'] = $wpGrade_Options->get('portfolio_archive_limit') ? absint($wpGrade_Options->get('portfolio_archive_limit')) : -1;
    }
    return $query;
}
...
function wpgrade_display_portfolio( $posts_nr , $front_page = false, $featured_first = true ) {
    ...
    if ( $featured_first ) {
        $query_args['meta_key'] = WPGRADE_PREFIX .'portfolio_featured';
	...
	<?php if ( !$front_page ) {
		...
		//lets do pagination
		global $wp_rewrite,$wpGrade_Options;
		$max_page = $query->max_num_pages;
		
		if ( !$paged ) $paged = 1;
		
		//test to see what kind of pagination one wants
		if ($wpGrade_Options->get('portfolio_ajax_loading_pagination')) {
			if ($max_page > $paged) { ?>
			<div class="portfolio-navigation clearfix">
				<div class="load_more"><a href="#"><?php echo $wpGrade_Options->get('portfolio_ajax_loading_text') ?></a></div>
				<?php
		...
		?>
	<div class="row portfolio-navigation clearfix" id="portfolio-nav">
        ...
        case 1: ?>
            <div class="block block1 block-white lap-push4">
                <div class="block-inner arrow arrow-left">
                    ...
                    <div class="portfolio-item_desc"><?php wpgrade_display_content( $content ); ?></div>
...
function wpgrade_get_portfolio_video( $videos ) {
    ...
    foreach ( $videos as $k => $video ){
        if ( preg_match( '/embed/', $k ) && !empty($video) ) {
            ...
            $video_webm = $video; //get_post_meta($postID, WPGRADE_PREFIX.'video_webm', true);
        } elseif ( preg_match( '/ogv/', $k ) ) {
            $video_ogv =  $video; //get_post_meta($postID, WPGRADE_PREFIX.'video_ogv', true);
        } elseif ( preg_match( '/preview/', $k ) ) {
            $video_poster =  $video; //get_post_meta($postID, WPGRADE_PREFIX.'video_poster', true);
        }
    }
