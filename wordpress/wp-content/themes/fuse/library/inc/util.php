...
if ( ! class_exists( 'util' ) ) {
    ...
    | (?<=[\xF0-\xF7][\x80-\xBF])[\x80-\xBF](?![\x80-\xBF]) # Short 4 byte sequence (2)
		...
		//remove the prefix the fast way - much faster than preg_match
		public static function replace_prefix ($prefix, $replace, $string) {
			if (substr($string, 0, strlen($prefix)) == $prefix) {
				$string = $replace . substr($string, strlen($prefix));
			}
