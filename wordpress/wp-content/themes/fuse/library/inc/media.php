...
if ( !function_exists( 'wpGrade_audio_selfhosted' ) )
{
    function wpGrade_audio_selfhosted($postID)
    {
        ...
        $audio_mp3 = get_post_meta($postID, WPGRADE_PREFIX.'audio_mp3', TRUE);
        $audio_m4a = get_post_meta($postID, WPGRADE_PREFIX.'audio_m4a', TRUE);
        $audio_oga = get_post_meta($postID, WPGRADE_PREFIX.'audio_ogg', TRUE);
        $audio_poster = get_post_meta($postID, WPGRADE_PREFIX.'audio_poster', true);
...
if ( !function_exists( 'wpGrade_video_selfhosted' ) ) {
    ...
    function wpGrade_video_selfhosted($postID) {
        ...
        $video_m4v = get_post_meta($postID, WPGRADE_PREFIX.'video_m4v', true);
        $video_webm = get_post_meta($postID, WPGRADE_PREFIX.'video_webm', true);
        $video_ogv = get_post_meta($postID, WPGRADE_PREFIX.'video_ogv', true);
        $video_poster = get_post_meta($postID, WPGRADE_PREFIX.'video_poster', true); ?>
...
function wpgrade_post_videos_ids($post_id)
{
    ...
    $vembed = get_post_meta($post_id, WPGRADE_PREFIX.'vimeo_link', true);
    if (preg_match('#(src=\"[^0-9]*)?vimeo\.com/(video/)?(?P<id>[0-9]+)([^\"]*\"|$)#', $vembed, $vmatches))
    {
        $result['vimeo'] = $vmatches["id"];
    }

    $yembed = get_post_meta($post_id, WPGRADE_PREFIX.'youtube_id', true);
...
if(!function_exists('wpGrade_gallery_slideshow'))
{
	function wpGrade_gallery_slideshow($current_post)
	{
		...
		if(!empty($match_gallery))
		{
			...
			//output the gallery
            echo '<div class="wp-slider loading gallery_format_slider">';
            echo '<div class="preloader"></div>';
			$temp =  do_shortcode($gallery);
			...
			foreach($divs as $div) {
				$div->setAttribute('class', 
				    $div->getAttribute('class') . ' slides');

                for( $i=0; $i < $div->childNodes->length; $i++){
                    $cn = $div->childNodes->item($i);
                    if( $cn->nodeType == 1){
                        $cn->setAttribute('class', 
                        $cn->getAttribute('class') . ' slide');
                   }
                }
			}
			echo $dom->saveHTML();
