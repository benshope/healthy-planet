...
function update_notifier_menu() {
	...
	if ($wpGrade_Options->get('themeforest_upgrade'))
	{
...
function update_notifier() {
	...
	var wpGradeUpdateData = {
		...
		envatoDetails: <?php if ($wpGrade_Options->get('marketplace_username') && $wpGrade_Options->get('marketplace_api_key')) { echo "true"; } else { echo "false" ; } ?>
	};
	</script>
	...
	<div id="message" class="updated below-h2"><p><strong><?php echo $xml->message; ?></strong> You have version <?php echo WPGRADE_VERSION; ?> installed. Please update to version <?php echo $xml->latest; ?>.</p></div>
	...
	<div class="two-columns">
		...
		<?php if ( $wpGrade_Options->get('themeforest_upgrade_backup')) { ?>
			<br /><br /><b>You have activated the automatic theme backups!</b><br />
			The directory the backups get saved to is <br /><code>wp-content/envato-backups</code>.<br />
		<?php } ?>
		...
		if (!empty($marketplace_username) && !empty($marketplace_api_key))
		{
			...
			if ( !$wpGrade_Options->get('themeforest_upgrade_backup') || $upgrader->backup_theme( $theme_name ) === true ) {
				...
				if (isset($res->errors)) {
					...
					if ( $wpGrade_Options->get('themeforest_upgrade_backup')) {
						/* Theme Backup URI */
						$wpGrade_data->backup_uri = $upgrader->get_theme_backup_uri( $theme_name );
					}
				}
			} else {
				$wpGrade_data->backup_failed = true;
			}
		}
	}
}
	...
	if ($wpGrade_Options->get('themeforest_upgrade'))
	{
		...
		if(isset($wpGrade_data->theme_updated) && isset($_GET['wpGrade_update']) &&  $_GET['wpGrade_update']=='true'){
			if($wpGrade_data->theme_updated){
				$message = __('The theme has been updated successfully', wpGrade_txtd);
				...
				$message = __('An error occurred, the theme has not been updated. Please try again later or install the update manually.', wpGrade_txtd);
				if (isset($wpGrade_data->theme_update_error) && isset($wpGrade_data->theme_update_error[1])) {
					$message .= '<br/>('.__('Error message:', wpGrade_txtd).' '.$wpGrade_data->theme_update_error[1].')';
				}
				$message_type = "error";
			}
		}elseif(isset($wpGrade_data->curl_disabled) && $wpGrade_data->curl_disabled === true){
			$message = __('Error: The theme was not updated, because the cURL extension is not enabled on your server. In order to update the theme automatically, the Envato Toolkit Library requires cURL to be enabled on your server. You can contact your hosting provider to enable this extension for you.', wpGrade_txtd);
			$message_type = "error";
