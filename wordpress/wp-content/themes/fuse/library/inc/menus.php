...
if(!function_exists(' wpgrade_register_custom_menus'))
{
	function wpgrade_register_custom_menus(){
		...
		$wpGrade_Options->set('nav_menus', 
			array(
				'main_menu' => __( 'Header Menu', wpGrade_txtd ),   // main nav in header
			));
		
		add_theme_support( 'menus' );
		
		foreach($wpGrade_Options->get('nav_menus') as $key => $value)
		{ 
			register_nav_menu($key, WPGRADE_THEMENAME.' '.$value);
		}
	}
