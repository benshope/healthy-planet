		...
		if(empty($socialimg)) 
		{
			...
			else 
			{
				...
				else
				{
					...
					if ($wpGrade_Options->get('social_share_default_image'))
					{
						$socialimg = $wpGrade_Options->get('social_share_default_image');
					}
					else
					{
...
function get_portfolio_excerpt($postID){
	//we need to get the first row content as the excerpt
	$rows = get_post_meta( $postID, WPGRADE_PREFIX .'portfolio_rows', true);
	...
	if ( !empty($rows) ) {
		...
		foreach ( (array)$rows[0] as $k => $field ) {
			// look for editor
...
function wpgrade_facebook_opengraph()
{
    ...
    if ( $wpGrade_Options->get( 'facebook_id_app' ) ) {
        echo '<meta property="fb:app_id" content="'.$wpGrade_Options->get( 'facebook_id_app' ).'"/>' . "\n";
    }
    if ( $wpGrade_Options->get( 'facebook_admin_id' ) ) {
        echo '<meta property="fb:admins" content="'.$wpGrade_Options->get( 'facebook_admin_id' ).'"/>';
    }
	...
	if (is_singular())
	{
		...
		if ($post->post_type == 'portfolio') {
			echo '<meta property="og:description" content="'.  strip_tags(get_portfolio_excerpt($post->ID)).'" />' . "\n";
		} else {
...
function wpgrade_google_metas()
{
	...
	if (is_singular())
	{
		...
		echo '" />' . "\n";
		if ($post->post_type == 'portfolio') {
			echo '<meta itemprop="description" content="'.  strip_tags(get_portfolio_excerpt($post->ID)).'" />' . "\n";
		} else {
	...
	if (is_front_page() && $wpGrade_Options->get( 'google_page_url' ) ) {
            echo '<link rel="publisher" href="http://plus.google.com/' . $wpGrade_Options->get( 'google_page_url') . '" />';
    }
...
function wpgrade_twitter_card()
{
    ...
    if (is_singular())
    {
		...
		echo '" />' . "\n";
       if ($post->post_type == 'portfolio') {
			echo '<meta name="twitter:description" content="'.  strip_tags(get_portfolio_excerpt($post->ID)).'" />' . "\n";
		} else {
...
function load_social_share() {
    ...
    if ( $wpGrade_Options->get('prepare_for_social_share') ) {
